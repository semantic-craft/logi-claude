namespace Loupedeck.CodexActionRingPlugin.Feedback.Tests
{
    ...
    {
        private static IEnumerable<RingActionId> CodexDirectedActions =>
            Enum.GetValues<RingActionId>();

        [Fact]
        public void EveryCodexDirectedActionUsesTheSameHonestFourStatePolicy()
        {
            foreach (var actionId in CodexDirectedActions)
            {
        ...
        public void InvalidInputsFailSilent()
        {
            ...
            Assert.Equal(
                FeedbackDecision.None,
                FeedbackPolicy.Decide(RingActionId.NewChat, (DispatchResult)Int32.MaxValue, FeedbackPresentationContext.Selection));
            Assert.Equal(
                FeedbackDecision.None,
                FeedbackPolicy.Decide(RingActionId.NewChat, DispatchResult.DispatchRequested, (FeedbackPresentationContext)Int32.MaxValue));
        }
