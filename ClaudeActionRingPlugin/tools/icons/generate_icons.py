#!/usr/bin/env python3
"""Generate deterministic Claude Action Ring icon projections and review sheets."""
...
BRAND_ICON = ICON_ROOT / "brand" / "claude-action-ring.svg"
SLOT_COLOR = "#D97757"  # matches backgroundColor in LoupedeckPackage.yaml
...
def make_ring_sheet(theme: str) -> str:
    ...
    for index, key in enumerate(ACTION_KEYS):
        ...
        for center_x, opacity in ((normal_x, "1"), (unavailable_x, "0.38")):
            parts.append(
                f'<circle cx="{center_x}" cy="{center_y}" r="41" fill="{SLOT_COLOR}" opacity="{opacity}"/>'
            )
            parts.append(
...
def make_state_sheet() -> str:
    ...
    for index, (label, opacity, overlay) in enumerate(states):
        ...
        parts.append(f'<circle cx="{center_x}" cy="{center_y}" r="41" fill="{SLOT_COLOR}"/>')
        parts.append(
            transformed_geometry(
                "side_chat",
                size=54,
...
def generate(output: Path) -> None:
    ...
    plugin_svg = BRAND_ICON.read_text(encoding="utf-8")
    ...
    source_files = (
        ...
        + [ICON_ROOT / "state-projections.json", BRAND_ICON]
    )
...
def main() -> int:
    ...
    if args.check:
        ...
        with tempfile.TemporaryDirectory(prefix="claude-action-ring-icons-") as temp_dir:
