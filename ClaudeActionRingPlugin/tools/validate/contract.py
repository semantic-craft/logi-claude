#!/usr/bin/env python3
"""Deterministic, read-only contract checks for the Claude Action Ring package."""
    ...
    for root in roots:
        ...
        for path in root.rglob("*"):
            ...
            if path.suffix in {".pyc", ".pyo"} or path.name == ".DS_Store":
                continue
            yield path
    yield PLUGIN_ROOT / "ClaudeActionRingPlugin.sln"
...
def check_source_boundaries() -> dict[str, object]:
    ...
    if csproj_files != [PLUGIN_ROOT / "src" / "ClaudeActionRingPlugin.csproj"]:
    ...
    application = (PLUGIN_ROOT / "src" / "ClaudeActionRingApplication.cs").read_text()
    if (
        'GetProcessName() => "Claude"' not in application
        or 'GetBundleName() => "com.anthropic.claudefordesktop"' not in application
    ):
        raise ContractError("Claude application binding mismatch")
    ...
    return {
        ...
        "assembly": "ClaudeActionRingPlugin.dll",
        "targetFramework": "net10.0",
        "application": {"process": "Claude", "bundleId": "com.anthropic.claudefordesktop"},
...
def privacy_categories(payload: bytes) -> list[str]:
    ...
    if any(literal in lowered for literal in literals) or re.search(
        rb"/users/[^/]+/(?:projects|\.claude)/", lowered
    ):
    ...
    patterns = {
        ...
        "approval-actions": rb"\b(?:approve|decline)\b",
    }
    categories.update(
