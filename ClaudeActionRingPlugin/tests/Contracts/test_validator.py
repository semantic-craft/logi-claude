...
SPEC = importlib.util.spec_from_file_location(
    "claude_action_ring_validation_contract", CONTRACT_PATH
)
...
class ValidatorContractTests(unittest.TestCase):
    ...
    @unittest.skipUnless(
        CONTRACT_PATH.parents[2].joinpath("artifacts").is_dir(),
        "release artifact not packed yet",
    )
    ...
    def test_privacy_scan_reports_categories_not_secret_values(self) -> None:
        ...
        self.assertEqual(
            ["approval-actions", "credential", "email", "private-path-or-user"],
