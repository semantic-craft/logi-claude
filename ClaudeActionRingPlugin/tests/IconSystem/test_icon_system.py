...
class RasterAndReviewContracts(unittest.TestCase):
    def test_plugin_icon_is_rgba_256_and_inside_safe_area(self) -> None:
        ...
        with Image.open(path) as image:
            ...
            self.assertEqual(image.getpixel((128, 50))[:3], (217, 119, 87))
            self.assertTrue(
                any(
    ...
    def test_plugin_icon_uses_independent_brand_master(self) -> None:
        brand = GENERATOR.BRAND_ICON.read_text()
        ...
        self.assertEqual(
            GENERATOR.sha256(GENERATOR.BRAND_ICON),
            manifest["sourceSha256"]["brand/claude-action-ring.svg"],
        )
        self.assertNotIn("<text", generated)
        self.assertNotEqual(
            GENERATOR.svg_inner(MASTER_ROOT / "side_chat.svg"),
            GENERATOR.svg_inner(GENERATOR.BRAND_ICON),
        )
